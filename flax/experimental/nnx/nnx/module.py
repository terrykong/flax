...
from flax.experimental.nnx.nnx import containers, errors, ids, nodes, partitioning, reprlib, tracers
from flax.experimental.nnx.nnx.containers import Container, Node, Sharding
...
class ModuleDef(tp.Generic[M], reprlib.Representable):
  ...
  def __init__(
      ...
      index: int,
      submodules: tp.Tuple[
          tp.Tuple[str, tp.Union["ModuleDef[Module]", int]], ...
      ],
  ...
  def update_state(
    ...
    if isinstance(updates, Module):
      ...
      if isinstance(updates.states, tuple):
        ...
        raise TypeError(
            "Expected Module, PureModule, State or tuple of State, "
...
def _make_module_def_recursive(
  ...
  for name, value in sorted(vars(module).items(), key=lambda x: x[0]):
    ...
    if isinstance(value, Module):
      submodule_def = _make_module_def_recursive(
          value, module_index, value_path
      )
...
def _update_module(
    module: Module,
    current_state: State,
    updates: tp.Union[State, tp.Tuple[State, ...]],
) -> None:
