...
from flax.experimental.nnx.nnx import errors, tracers
...
jtu.register_pytree_node(
    PureContext, _pure_context_flatten, _pure_context_unflatten
)
...
class Context:
  ...
  def partition(self) -> PureContext:
    ...
    rng_counts = tuple(
        (name, stream.count_path) for name, stream in rngs.items()
    )
    return PureContext.new(
        keys, ContextDef(rng_counts, tuple(self._flags.items()))
    )
...
def context(
  ...
  _rngs = {
      name: (
          RngStream(jax.random.PRNGKey(value))
...
RngFilter = tp.Union[
    RngFilterLiteral,
    tp.Sequence[RngFilterLiteral],
    tp.Mapping[RngFilterLiteral, bool],
]
...
def to_rng_predicate(filter: RngFilter) -> RngPredicate:
  if filter is None:
    ...
    predicates = tuple(
        to_rng_predicate(filter)
        for filter, include in filter.items()
        if include
    )
