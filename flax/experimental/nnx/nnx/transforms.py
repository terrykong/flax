...
from flax.experimental.nnx.nnx import containers, contextlib, partitioning, spmd, tracers
...
def value_and_grad(
    ...
    *,
    ...
    ...,
    tp.Union[
        tp.Tuple[tp.Tuple[jax.Array, tp.Any], State], tp.Tuple[jax.Array, State]
    ],
]:
  predicate = partitioning.to_predicate(wrt)
  ref_grad = GradTransform(
...
def scan_apply(
  ...
  def scan_fn(
    ...
    # split module state
    (*axes_states_out, broadcast_state_out, carry_state_out), _ = (
        module.partition(*filters)
    )
    ...
    if spmd.PARTITION_NAME in options.metadata_params:
      axes_states_out = [
          ...
          for state, index in zip(
              axes_states_out, options.variable_axes.values()
          )
      ]
  ...
  abstract_output = jax.eval_shape(
      scan_partial(length, options.unroll), carry, axes
  )
...
def remat(
  ...
  if is_init:
    ...
    @functools.wraps(f)
    def wrapper(
        module: Module, *args, ctx: tp.Optional[contextlib.Context] = None
    ):
