...
from flax.experimental.nnx.nnx import nodes, reprlib
...
class Container(
    tp.Generic[A], reprlib.Representable, metaclass=ContainerMetaclass
):
  value: A

  def __init__(
      self, value: tp.Union[A, ContainerMetadata[A]], **metadata: tp.Any
  ):
  ...
  def is_equivalent(self, other: tp.Any) -> bool:
    ...
    return type(self) is type(other) and metadata_fields(
        self
    ) == metadata_fields(other)
